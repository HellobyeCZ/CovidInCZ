...
output: 
  flexdashboard::flex_dashboard:
    ...
    runtime: shiny 
    navbar:
      - {href: "https://github.com/HellobyeCZ", icon: "ion-social-github"}
---
...
fit.model <- subset(minus, as.Date(minus$datum) >= as.Date("2020-12-01"))
fdate <- 1:nrow(fit.model)
fit.model <- cbind(fit.model,fdate)


fit1 <- lm(log(kumulativni_pocet_nakazenych) ~ fdate,data = fit.model) #estimation of infect ratio
fit2 <- lm(log(kumulativni_pocet_vylecenych) ~ fdate,data = fit.model) #estimation of recovery ratio
fit3 <- lm(log(kumulativni_pocet_umrti) ~ fdate,data = fit.model) #estimation of death ratio




beta <- coef(fit1)[2]/1000009 #infect ratio
gamma <- coef(fit2)[2] #recovery 
theta <- coef(fit3)[2] #umrtnost
lambda <- zaklad.stat$aktualne_hospitalizovani/zaklad.stat$aktivni_pripady #hospitalizovan??
S0 <- 10000000
I0 <- 38857
R0 <- 18933
D0 <- 8626
...
times <- seq(0, 250)
kapacita <- 15500
datumy <- seq.Date(as.Date("2020-12-01"), by = 1, along.with = times)
...
minusV <- minus$new[nakaza$datum >= "2020-12-01"]
datumZ <- nakaza$datum[nakaza$datum >= "2020-12-01"]
minusD <- minus$kumulativni_pocet_umrti[nakaza$datum >= "2020-12-01"]
datazariV <- tibble(as.factor(datumZ), minusV,minusD)
colnames(datazariV) <- c("datum", "Actual", "Deaths")
data <- tibble(data,H, datumy)
...
```{r}
ggplotly(ggplot(datazariV, aes(as.Date(datum),Actual)) +
  geom_line() +
  xlab("Date") +
  ylab("Current number of active cases") + 
  theme_minimal() + 
